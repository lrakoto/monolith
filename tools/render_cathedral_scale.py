...
scene.render.image_settings.file_format='PNG';scene.render.filepath=str(OUT/'cathedral-scale-study.png')
...
mon=bpy.data.objects.new('limestone monument',me);scene.collection.objects.link(mon);mon.location=(0,240,60);mon.scale=(2.2,2.2,2.2);me.materials.append(stone)
b=mon.modifiers.new('soft limestone edges','BEVEL');b.width=.09;b.segments=3
box('landing',(0,234,58.5),(32,18,3),stairmat,.08)
box('dark entrance',(0,228.48,61.4),(1.8,.15,2.8),bark)
# a small dark inset makes the actual engraved letters legible at this distance.
font=bpy.data.curves.new('305 engraved shadow','FONT');font.body='305';font.size=2.8;font.align_x='CENTER';font.extrude=.002
letter=bpy.data.objects.new('305',font);scene.collection.objects.link(letter);letter.location=(0,228.48,66);letter.rotation_euler=(math.pi/2,0,0);font.materials.append(stairmat)
for i in range(300):
 h=(i+1)*60/300;y=18+(i+.5)*210/300;w=7.6-i/300*2.1
 box('tread %03d'%i,(0,y,h/2),(w,210/300+.01,h),stairmat,.018)
# ground rolls into banks, leaving a submerged bed across the foreground.
def height(x,y):
 rise=max(0,min(1,(y-18)/210))*60
 bank=max(0,min(1,(abs(x)-3.4)/9));bank=bank*bank*(3-2*bank)
...
for i in range(80):trees.append(((1 if i%2 else -1)*(25+random.random()*66),35+random.random()*360,.4+random.random()*.7))
...
centers=[]
for i in range(420):
 y=10+random.random()*265;x=(1 if i%2 else -1)*(7+random.random()*35);r=2+random.random()*4
...
for i in range(24):
 t=i/23;y=20+t*206;z=(y-18)*60/210
 for side in [-1,1]:
  x=side*(4.05-t*1.05)
...
area('entrance warmth',(0,224,63),(0,231,63),350,2,(1,.52,.27))
area('overcast opening',(-25,100,260),(0,130,25),450000,150,(.76,.86,.79))
area('limestone soft light',(-35,170,190),(0,240,140),130000,90,(.83,.88,.81))
...
bpy.ops.object.camera_add(location=(0,-35,1.15));camera=bpy.context.object;camera.name='reference camera';camera.data.lens=34;camera.data.clip_end=800;aim(camera,(0,230,66));scene.camera=camera
scene.render.film_transparent=False
bpy.context.preferences.filepaths.save_version=0
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-scale-study.blend'))
