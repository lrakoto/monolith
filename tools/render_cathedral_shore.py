"""Water margin: the difference map puts the worst errors in the pond and the lit shorelines."""
...
STILL=Path(QUICK) if QUICK else OUT/'cathedral-shore-study.png'
random.seed(1707)
...
def mat(name,low,high,scale=5,bump=.1,rough=.85,stretch=None):
 ...
 return m
bark=mat('damp deeply furrowed bark',(.006,.011,.007),(.040,.050,.028),3,.23,.94,(2,2,.13))
...
if not QUICK:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-shore-study.blend'))
