"""Fine instanced leaves over the approved crown masses and reference composition."""
...
scene.render.image_settings.file_format='PNG';scene.render.filepath=str(OUT/'cathedral-foliage-study.png')
...
for family,source in enumerate(crown_meshes):
 ...
 for j in range(11000):
  ...
  side=normal.cross(forward).normalized();length=leaf_rng.uniform(.035,.065)
  base=len(vertices)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-foliage-study.blend'))
