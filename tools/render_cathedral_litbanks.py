"""Lit banks: the reference lights its right bank and near field hard, and crowds the left corner."""
...
STILL=Path(QUICK) if QUICK else OUT/'cathedral-litbanks-study.png'
random.seed(1707)
...
def mat(name,low,high,scale=5,bump=.1,rough=.85,stretch=None,gen=False):
 ...
 return m
bark=mat('vine draped colossal bark',(.0004,.0006,.0004),(.0094,.0112,.0068),1,.16,.985,(55,55,2.0),True)
...
leaf_materials=[]
for j,color in enumerate([(.0117,.0282,.0109),(.0208,.0427,.0126),(.0336,.0563,.0181),(.0463,.069,.0237)]):
...
for j,color in enumerate([(.108,.22,.097),(.177,.307,.117),(.270,.400,.158),(.369,.499,.201)]):
...
sunlit_material=mat('sunlit shoreline canopy',(.0426,.0803,.0377),(.2007,.2744,.1352),1,.2)
...
for i in range(165):
 ...
 if 34<i<112:
  ...
  x=-pad_rng.uniform(62,118);y=pad_rng.uniform(-104,-46);r=pad_rng.uniform(2.3,4.8)
...
for idx,loc,scale,rot in crown_specs:
 ...
 if loc[1]<365 and abs(loc[0])<265:
  foliage=(sunlit_leaf_meshes if loc[0]>0 else near_leaf_meshes) if loc[1]<12 else leaf_meshes
...
for idx,loc,scale,rot in shore_specs:
 ...
 if loc[0]>0:bank=sunstruck_meshes if loc[2]>height(loc[0],loc[1])+8 else sunlit_meshes
...
for idx,loc,scale,rot,lit in cling_specs:
 ...
 o=bpy.data.objects.new('trunk cling %03d'%idx,(sunlit_meshes if lit else crown_meshes)[family]);scene.collection.objects.link(o)
 o.location=loc;o.scale=scale;o.rotation_euler=rot
...
if not QUICK:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-litbanks-study.blend'))
