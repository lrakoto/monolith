"""Plinth: the reference flanks the tower with a broad lit wall where we leave dark forest."""
...
STILL=Path(QUICK) if QUICK else OUT/'cathedral-plinth-study.png'
random.seed(1707)
...
padmat=mat('lily pad waxy surface',(.045,.14,.022),(.30,.46,.10),14,.006,.4)
...
for i in range(165):
 ...
 if near:x=side*pad_rng.uniform(40,205);y=pad_rng.uniform(-240,-100)
 else:x=side*pad_rng.uniform(15,150);y=pad_rng.uniform(-150,-5)
 if i>157:x=pad_rng.uniform(-9,9);y=pad_rng.uniform(-130,-20)
 r=pad_rng.uniform(.6,1.7)*(1.7 if near else 1);rot=pad_rng.random()*math.tau;vs=[(x,y,.075)]
 for j in range(31):
...
for idx,loc,scale,rot in shore_specs:
 ...
 o=bpy.data.objects.new('shoreline stand %03d'%idx,shore_meshes[family]);scene.collection.objects.link(o)
...
if not QUICK:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-plinth-study.blend'))
