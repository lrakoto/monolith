"""Value structure: the reference is dark at the edges with a bright centre and a pale pond."""
...
scene.render.image_settings.file_format='PNG';scene.render.filepath=str(OUT/'cathedral-value-study.png')
...
ramp=nodes.new('ShaderNodeValToRGB');ramp.color_ramp.elements[0].color=(.018,.031,.037,1);ramp.color_ramp.elements[1].color=(.042,.061,.065,1)
...
leaf_materials=[]
for j,color in enumerate([(.022,.055,.020),(.04,.083,.025),(.065,.11,.035),(.09,.135,.045)]):
...
water=bpy.data.materials.new('deep green clear water');water.use_nodes=True;p=water.node_tree.nodes.get('Principled BSDF');# a deep transmissive pond read as the darkest thing in frame; in the reference it is the
# palest, a milky green that carries light back into the bottom of the composition.
p.inputs['Base Color'].default_value=(.17,.30,.23,1);p.inputs['Roughness'].default_value=.22;p.inputs['IOR'].default_value=1.333;p.inputs['Transmission Weight'].default_value=.22
...
# the broad fill flattened the frame to one mid green and lit the trunks like pale columns.
# the reference keeps its light in a central shaft and lets the edges go dark, so fill drops.
soft('open sky',(-100,80,650),1150000,250,(.76,.85,.88))
...
soft('left canopy opening',(-65,95,160),70000,55,(.83,.88,.72))
soft('right canopy opening',(85,235,265),75000,65,(.66,.79,.83))
soft('stair skylight',(0,130,190),300000,90,(.78,.85,.86))
soft('near bank opening',(100,5,110),105000,35,(.83,.89,.73))
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-value-study.blend'))
