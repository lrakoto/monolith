"""Tone: the three surfaces the map still has running bright are the trunks, tower and backdrop."""
...
STILL=Path(QUICK) if QUICK else OUT/'cathedral-tone-study.png'
random.seed(1707)
...
def mat(name,low,high,scale=5,bump=.1,rough=.85,stretch=None,gen=False):
 ...
 return m
bark=mat('vine draped colossal bark',(.0007,.0011,.0008),(.0172,.0205,.0124),1,.16,.985,(55,55,2.0),True)
...
ramp=nodes.new('ShaderNodeValToRGB');ramp.color_ramp.elements[0].color=(.0072,.0122,.0143,1);ramp.color_ramp.elements[1].color=(.0158,.023,.0244,1)
...
for group in range(54):
 ...
 for j in range(30):
  ...
  z=height(x,y)+r*.72+r*stand_rng.uniform(0,2.1)
...
for group in range(48):
 ...
 for j in range(26):
  x=under_rng.gauss(cx,17);y=under_rng.gauss(cy,23)
  if y<0 or y>345 or abs(x)<34-y/330*4:continue
...
for idx,loc,scale,rot in crown_specs:
 ...
 if loc[1]<365 and abs(loc[0])<265:
  foliage=near_leaf_meshes if loc[1]<12 else leaf_meshes
...
if not QUICK:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-tone-study.blend'))
