...
scene.render.image_settings.file_format='PNG';scene.render.filepath=str(OUT/'cathedral-landforms.png')
...
ramp.color_ramp.elements[0].position=.25;ramp.color_ramp.elements[0].color=(.065,.075,.064,1)
 ...
 for row in range(62):
  ...
  for col in range(22):
   ...
   for k in range(count):
    ...
    loc.z=max(height(loc.x,loc.y)+1.5,c.z+z)
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-landforms.blend'))
