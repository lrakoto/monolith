"""Cling: the trunk foliage was hanging beside the trunk rather than on it."""
...
STILL=Path(QUICK) if QUICK else OUT/'cathedral-cling-study.png'
random.seed(1707)
...
def mat(name,low,high,scale=5,bump=.1,rough=.85,stretch=None,gen=False):
 ...
 return m
bark=mat('vine draped colossal bark',(.0005,.0008,.0005),(.0158,.0188,.0114),1,.16,.985,(55,55,2.0),True)
...
if not QUICK:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-cling-study.blend'))
