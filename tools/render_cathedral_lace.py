"""Lace: the reference crowns are broken and lacy at the edge where ours stay closed."""
...
STILL=Path(QUICK) if QUICK else OUT/'cathedral-lace-study.png'
random.seed(1707)
...
for family in range(7):
 ...
 for k,(center,radius,stretch) in enumerate(lobes):
  ...
  for vert in result['verts']:
   vert.co*=1+noise_vector(vert.co*9+Vector((k,family,0))).x*.21+noise_vector(vert.co*23+Vector((k,family,4))).y*.105
...
vine_material=mat('hanging vine',(.0075,.0182,.0091),(.0405,.0672,.0289),1,.2)
vine_rng=random.Random(30727)
vine_verts=[];vine_faces=[]
def vine_leaf(centre,normal,forward,length):
 side=normal.cross(forward).normalized();base=len(vine_verts)
 vine_verts.extend([tuple(centre+normal*length*.12),tuple(centre-forward*length*.5),
                    tuple(centre+side*length*.22),tuple(centre+forward*length*.5),tuple(centre-side*length*.22)])
 for k in range(4):vine_faces.append((base,base+1+(k+1)%4,base+1+k))
for tx,ty,tr_r,zlo,zhi,profile in trunk_info:
 for j in range(430):
  t0=vine_rng.uniform(.24,.93)
  z0=zlo+t0*(zhi-zlo)
  here=profile[min(len(profile)-1,int(t0*len(profile)))]
  ang=vine_rng.random()*math.tau
  out=Vector((math.cos(ang),math.sin(ang),0))
  start=Vector((tx,ty,z0))+out*here*.995
  length=vine_rng.uniform(26,104);segments=13
  width=vine_rng.uniform(.17,.38)
  drift=Vector((vine_rng.gauss(0,.10),vine_rng.gauss(0,.10),0))
  ring=None
  for k in range(segments+1):
   f=k/segments
   centre=start+Vector((0,0,-length*f))+out*length*f*.06+drift*length*f
   w=width*(1-.72*f)
   a=out;b=Vector((-out.y,out.x,0))
   base=len(vine_verts)
   for m in range(3):
    theta=m*math.tau/3
    vine_verts.append(tuple(centre+(a*math.cos(theta)+b*math.sin(theta))*w))
   if ring is not None:
    for m in range(3):vine_faces.append((ring+m,ring+(m+1)%3,base+(m+1)%3,base+m))
   ring=base
   for _ in range(2 if k else 0):
    radial=(a*math.cos(vine_rng.random()*math.tau)+b*math.sin(vine_rng.random()*math.tau)).normalized()
    drop=Vector((0,0,-1))*vine_rng.uniform(.6,1)+radial*vine_rng.uniform(-.3,.3)
    vine_leaf(centre+radial*w*1.1+Vector((0,0,-vine_rng.uniform(0,length/segments))),
              radial,drop.normalized(),vine_rng.uniform(.85,1.95))
vine_mesh=bpy.data.meshes.new('hanging vines');vine_mesh.from_pydata(vine_verts,[],vine_faces);vine_mesh.update()
vine_mesh.materials.append(vine_material)
vine_object=bpy.data.objects.new('hanging vines',vine_mesh);scene.collection.objects.link(vine_object)
...
reed_specs=[]
for j in range(64):
 x=reed_rng.uniform(62,112);y=reed_rng.uniform(-108,-38)
...
soft('near bank opening',(88,-30,74),58000,22,(.83,.89,.73))
...
for idx,loc,scale,rot in crown_specs:
 ...
 emergent=loc[1]<640 and loc[2]-height(loc[0],loc[1])>19 and (loc[0]<26 or loc[2]<58)
...
for idx,loc,scale,rot in shore_specs:
 ...
 if loc[0]>0:bank=sunstruck_meshes if loc[2]>height(loc[0],loc[1])+6 else sunlit_meshes
...
if not QUICK:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-lace-study.blend'))
