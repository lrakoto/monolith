"""Foreground: the reference near field is lit and pale where ours stays in the same dark tone."""
...
STILL=Path(QUICK) if QUICK else OUT/'cathedral-foreground-study.png'
random.seed(1707)
...
scene.render.resolution_x=scene.render.resolution_y=600 if QUICK else 1200;scene.render.resolution_percentage=100
scene.render.image_settings.file_format='PNG';scene.render.filepath=str(STILL)
scene.world.use_nodes=True;scene.world.node_tree.nodes['Background'].inputs['Color'].default_value=(.25,.36,.32,1);scene.world.node_tree.nodes['Background'].inputs['Strength'].default_value=.62
...
def mat(name,low,high,scale=5,bump=.1,rough=.85,stretch=None,gen=False):
 ...
 return m
bark=mat('vine draped colossal bark',(.0011,.0018,.0012),(.032,.038,.023),1,.16,.985,(55,55,2.0),True)
...
bark_ramp.color_ramp.elements[0].position=.37;bark_ramp.color_ramp.elements[1].position=.635
moss=mat('moss and forest earth',(.008,.018,.006),(.045,.087,.025),3,.14)
stone=mat('weathered pale limestone',(.23,.27,.24),(.56,.59,.51),2.5,.045)
plinth_stone=mat('weathered plinth limestone',(.155,.185,.165),(.385,.415,.355),2.5,.045)
...
for i,(x,y,r) in enumerate([(-175,155,90),(200,280,115)]):
 ...
 trunk_info.append((x,y,r,tr.location.z+min(trunk_zs)*tr.scale.z,tr.location.z+max(trunk_zs)*tr.scale.z))
...
ramp=nodes.new('ShaderNodeValToRGB');ramp.color_ramp.elements[0].color=(.010,.017,.020,1);ramp.color_ramp.elements[1].color=(.022,.032,.034,1)
...
canopy_material=mat('blockout canopy',(.007,.017,.009),(.042,.070,.030),1,.2)
crown_meshes=[]
...
leaf_materials=[]
for j,color in enumerate([(.013,.031,.012),(.023,.047,.014),(.037,.062,.020),(.051,.076,.026)]):
 material=bpy.data.materials.new('leaf tone %d'%j);material.use_nodes=True
 shader=material.node_tree.nodes.get('Principled BSDF')
 shader.inputs['Base Color'].default_value=(*color,1);shader.inputs['Roughness'].default_value=.67
...
for family,source in enumerate(crown_meshes):
 ...
 for j in range(7500):
  ...
  add_leaf(center,normal,forward,leaf_rng.uniform(.035,.065),leaf_rng.randrange(4))
...
shore_material=mat('lit shoreline canopy',(.030,.062,.028),(.155,.235,.105),1,.2)
sunlit_material=mat('sunlit shoreline canopy',(.052,.098,.046),(.245,.335,.165),1,.2)
shore_meshes=[];sunlit_meshes=[]
...
cling_specs=[]
for tx,ty,tr_r,zlo,zhi in trunk_info:
 for j in range(620):
  t=cling_rng.random()**.8
  z=zlo+60+t*(zhi-zlo-120)
  ang=cling_rng.random()*math.tau
  rad=tr_r*cling_rng.uniform(.90,1.0)*(1-.13*t)
  x=tx+math.cos(ang)*rad;y=ty+math.sin(ang)*rad
  r=cling_rng.uniform(1.8,5.5)*(1-.3*t)
  cling_specs.append((len(cling_specs),(x,y,z),(r,r*cling_rng.uniform(.8,1.2),r*cling_rng.uniform(.9,1.5)),(0,0,cling_rng.random()*math.tau)))
...
p.inputs['Base Color'].default_value=(.080,.138,.105,1);p.inputs['Roughness'].default_value=.22;p.inputs['IOR'].default_value=1.333;p.inputs['Transmission Weight'].default_value=.22
...
padmat=mat('lily pad waxy surface',(.072,.185,.034),(.395,.55,.135),14,.006,.4)
...
for i in range(165):
 ...
 if 40<i<96:
  ...
  x=-pad_rng.uniform(34,124);y=pad_rng.uniform(-125,-42);r=pad_rng.uniform(1.6,3.9)
 elif i>157:x=pad_rng.uniform(-9,9);y=pad_rng.uniform(-130,-20);r=pad_rng.uniform(.6,1.7)
...
for idx,loc,scale,rot in crown_specs:
 ...
 mesh=back_meshes[family] if loc[1]>650 else shore_meshes[family] if loc[1]<12 else crown_meshes[family]
 ...
 if loc[1]<365 and abs(loc[0])<265:
  leaves=bpy.data.objects.new('fine foliage %03d'%idx,leaf_meshes[family]);scene.collection.objects.link(leaves)
...
for idx,loc,scale,rot in shore_specs:
 ...
 bank=sunlit_meshes if loc[0]>0 else shore_meshes
...
for idx,loc,scale,rot in cling_specs:
...
if not QUICK:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-foreground-study.blend'))
