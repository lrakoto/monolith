"""Dark forest mass: the reference median sits near its shadow floor, ours sat mid frame."""
...
STILL=Path(QUICK) if QUICK else OUT/'cathedral-mass-study.png'
random.seed(1707)
...
p.inputs['Base Color'].default_value=(.16,.285,.215,1);p.inputs['Roughness'].default_value=.22;p.inputs['IOR'].default_value=1.333;p.inputs['Transmission Weight'].default_value=.22
...
if not QUICK:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-mass-study.blend'))
