/* the pond is framed from below: shrubs stay beneath the lantern sightline,
   emergent plants grow from the basin, and pads leave the reflection open. */
...
function buildPondGarden() {
  ...
  const shrubs=[[-10.2,2.6,1.3],[11.8,1.4,1.5],[-13,-8,1.7],[14,-10,1.6],[-15,-19,1.4],[17,-24,1.8],[-19,-32,1.5],[21,-37,1.7]];
  const twigs=[],locations=[];
  shrubs.forEach(([x,z,height])=>{
    const ground=Math.max(0,gardenGround(x,z));
    for(let branch=0;branch<7;branch++){
      const angle=rnd()*TAU,reach=.5+rnd()*.8;
      ...
      for(let j=0;j<(LOW?18:42);j++){
        const a=rnd()*TAU,r=Math.sqrt(rnd())*.68;
        locations.push(new THREE.Vector3(end.x+Math.cos(a)*r,end.y+(rnd()-.5)*.6,end.z+Math.sin(a)*r));
      }
    }
  });
  const wood=new THREE.Mesh(mergeGeos(twigs),gardenSway(GARDEN.bark.clone(),.15,2.4,.10));wood.name='low pond shrub branches';wood.receiveShadow=true;GARDEN.group.add(wood);
  const leafMat=gardenSway(gardenWind(new THREE.MeshStandardMaterial({color:0x93ac6d,side:THREE.DoubleSide,roughness:.82,envMapIntensity:.7}),.24,true),.15,2.4,.10);
