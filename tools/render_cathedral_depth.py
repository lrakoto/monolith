"""Simple forest masses and depth layers around the distant reference camera."""
...
scene.render.image_settings.file_format='PNG';scene.render.filepath=str(OUT/'cathedral-depth-study.png')
...
back.inputs['Color'].default_value=(.035,.052,.05,1);back.inputs['Strength'].default_value=.35
...
def height(x,y):
 ...
 return rise*bank-3+bank*shore*relief+noise_vector(Vector((x*.014,y*.014,2))).z*7*bank
verts=[];faces=[];N=90
for j in range(N+1):
 y=-360+j*1160/N
 for i in range(N+1):
...
for i,(x,y,r) in enumerate([(-175,155,90),(200,280,115)]):
 ...
 for vert in tr.data.vertices:
  ...
  radial=1+.055*math.sin(angle*7+t*14+i)+.035*math.sin(angle*13-t*5)
...
fade.inputs['From Min'].default_value=.24;fade.inputs['From Max'].default_value=.52
fade.inputs['To Min'].default_value=.00065/3;fade.inputs['To Max'].default_value=.0017
...
vol.node_tree.links.new(fade.outputs['Result'],v.inputs['Density'])
...
back_mesh=core.data.copy();back_mesh.materials.clear();back_mesh.materials.append(distant_bark)
for idx,loc,scale,rot in crown_specs:
 o=bpy.data.objects.new('canopy lobe %03d'%idx,back_mesh if loc[1]>650 else core.data);scene.collection.objects.link(o);o.location=loc;o.scale=scale;o.rotation_euler=rot
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-depth-study.blend'))
