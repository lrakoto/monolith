"""Emergent: in the reference the canopy tops catch light everywhere, not only in the near field."""
...
STILL=Path(QUICK) if QUICK else OUT/'cathedral-emergent-study.png'
random.seed(1707)
...
for idx,loc,scale,rot in crown_specs:
 ...
 emergent=loc[1]<640 and loc[2]-height(loc[0],loc[1])>19
...
if not QUICK:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-emergent-study.blend'))
