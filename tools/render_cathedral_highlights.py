"""Highlights: the reference has far more bright pixels than we do at the same average."""
...
STILL=Path(QUICK) if QUICK else OUT/'cathedral-highlight-study.png'
random.seed(1707)
...
canopy_material=mat('blockout canopy',(.0048,.0116,.0061),(.0286,.0476,.0204),1,.2)
crown_meshes=[]
...
leaf_materials=[]
for j,color in enumerate([(.0088,.0211,.0082),(.0156,.032,.0095),(.0252,.0422,.0136),(.0347,.0517,.0177)]):
...
shore_material=mat('lit shoreline canopy',(.021,.0434,.0196),(.1085,.1645,.0735),1,.2)
sunlit_material=mat('sunlit shoreline canopy',(.0364,.0686,.0322),(.1715,.2345,.1155),1,.2)
...
key=bpy.data.lights.new('canopy key','SUN');key.energy=1.82;key.angle=math.radians(5);key.color=(.93,.95,.86)
...
if not QUICK:bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-highlight-study.blend'))
