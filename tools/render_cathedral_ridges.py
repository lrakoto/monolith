...
scene.render.image_settings.file_format='PNG';scene.render.filepath=str(OUT/'cathedral-ridges.png')
scene.world.use_nodes=True;scene.world.node_tree.nodes['Background'].inputs['Color'].default_value=(.23,.31,.34,1);scene.world.node_tree.nodes['Background'].inputs['Strength'].default_value=.18
...
def area(name,loc,target,energy,size,color):
 d=bpy.data.lights.new(name,'AREA');d.energy=energy;d.shape='DISK';d.size=size;d.color=color;o=bpy.data.objects.new(name,d);scene.collection.objects.link(o);o.location=loc;o.visible_camera=False;aim(o,target);return o
...
def height(x,y):
 ...
 # separate the near shoulders from the high ridges with broad eroded pockets.
 notch=1-.30*math.exp(-((y-(145 if x<0 else 95))/32)**2)
 lowpeaks=[(-42,85,44,25,32),(-54,205,95,28,43),(50,140,78,28,36),(64,285,115,32,48)]
 ledges=max(amp*math.exp(-((x-cx)/wx)**2-((y-cy)/wy)**2) for cx,cy,amp,wx,wy in lowpeaks)
 inner=max(0,min(1,(abs(x)-17)/19));inner=inner*inner*(3-2*inner)
 shore=max(0,min(1,(y+22)/60));shore=shore*shore*(3-2*shore)
 return rise*bank+bank*(1.1+noise_vector(Vector((x*.07,y*.07,1))).z*1.8)-2.4*basin+shore*(shoulder*max(0,ridge*notch+erosion)+inner*ledges)
verts=[];faces=[];N=190
for j in range(N+1):
 y=-190+j*650/N
 for i in range(N+1):
...
for i in range(5):
 ...
 # broad color patches read as stands of trees rather than random leaves everywhere.
 n=m.node_tree.nodes;l=m.node_tree.links;geo=n.new('ShaderNodeNewGeometry');patch=n.new('ShaderNodeTexNoise');patch.inputs['Scale'].default_value=.025;patch.inputs['Detail'].default_value=2
 color=n.new('ShaderNodeValToRGB');color.color_ramp.elements[0].color=(.012+i*.003,.036+i*.006,.011+i*.002,1);color.color_ramp.elements[1].color=(.044+i*.006,.108+i*.009,.025+i*.004,1)
 l.new(geo.outputs['Position'],patch.inputs['Vector']);l.new(patch.outputs['Fac'],color.inputs[0]);l.new(color.outputs['Color'],p.inputs['Base Color'])
 leafm.append(m)
...
for variant in range(8):
 ...
 for j in range(900):
  ...
  radius=.83+noise_vector(radial*3+Vector((variant,0,0))).x*.37
 ...
 for row in range(119):
  ...
  for col in range(42):
   ...
   for k in range(density):
    ...
    if abs(xx)<16 or height(xx,yyy)<.5:continue
    stand=noise_vector(Vector((xx*.028,yyy*.028,12))).x
    # leave occasional shaded breaks around exposed rock, not evenly spaced holes.
    if stand<-.48 and dx*dx+dy*dy>5:continue
    size=random.uniform(1.5,3.15)*(.85+max(0,stand)*1.1)
...
water=bpy.data.materials.new('deep green clear water');water.use_nodes=True;p=water.node_tree.nodes.get('Principled BSDF');p.inputs['Base Color'].default_value=(.055,.18,.115,1);p.inputs['Roughness'].default_value=.12;p.inputs['IOR'].default_value=1.333;p.inputs['Transmission Weight'].default_value=.65
...
for i in range(65):
 ...
 r=random.uniform(.35,.9);rot=random.random()*math.tau;vs=[(x,y,.075)]
 for j in range(31):
...
area('soft pond daylight',(-65,-20,95),(0,50,12),85000,70,(.72,.84,.8))
area('water opening',(10,-48,42),(0,-20,0),26000,55,(.78,.88,.83))
area('left canopy break',(-12,45,150),(-58,110,80),33000,38,(.81,.87,.76))
area('right distant canopy',(8,160,230),(75,225,155),44000,55,(.68,.79,.81))
area('limestone soft light',(-35,170,240),(0,240,190),110000,110,(.83,.88,.81))
# broad point fills avoid the sharp cutoff of area lights in the mist.
for name in ['left canopy break','right distant canopy']:
 light=bpy.data.lights[name];light.type='POINT';light.shadow_soft_size=30
# a real participating medium joins the layers of trees and softens the distance.
vol=bpy.data.materials.new('forest atmosphere');vol.use_nodes=True;ns=vol.node_tree.nodes;ns.clear();v=ns.new('ShaderNodeVolumePrincipled');v.inputs['Density'].default_value=.0016;v.inputs['Color'].default_value=(.43,.56,.60,1);v.inputs['Anisotropy'].default_value=.35;out=ns.new('ShaderNodeOutputMaterial');vol.node_tree.links.new(v.outputs['Volume'],out.inputs['Volume'])
...
if '--draft' in sys.argv:
 scene.render.resolution_x=1000;scene.render.resolution_y=1000;scene.cycles.samples=48;scene.render.filepath=str(OUT/'cathedral-ridges-draft.png')
bpy.context.preferences.filepaths.save_version=0
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'cathedral-ridges.blend'))
