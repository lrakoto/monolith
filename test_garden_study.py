...
for(const kind of ['temple','redwoods','forest'])for(const LOW of [false,true]){
 ...
 if(kind==='forest'){assert.equal(g.children.filter(o=>o.name==='pond terrace tree').length,2);assert.equal(g.children.filter(o=>o.name==='pond backdrop tree').length,4);assert(g.getObjectByName('pond bank shrub branches'));assert(!g.getObjectByName('woodland tree'));assert(g.getObjectByName('emergent pond reeds').count>0);const pads=g.getObjectByName('pond lily pads');assert(pads.count>0);for(let i=0;i<pads.count;i++){const m=new THREE.Matrix4();pads.getMatrixAt(i,m);assert(Math.abs(m.elements[12])>=6.2);}}
