...
## Water shoals and graded clarity — 2026-09-22

The next four cloud studies test whether the reference's broken, green shallows can be made more legible without brightening the near foreground. They preserve the camera, architecture, forest, lights, and previous scene objects. `tools/prepare_cathedral_water_shoals.py` loads the shore-clarity scene, replaces its continuous submerged layer with 31 irregular shallow meshes, and saves `cathedral-water-shoals-cloud-v1.blend`. Job `2857480` ($0.20) shows the shoals are too faint through the grazing-angle water. Running the same builder with `-- --layered` retains the old continuous layer and adds 46 larger shoals in `cathedral-water-layered-shoals-cloud-v1.blend`; job `2857484` ($0.10) retains the green depth but the individual forms still hardly read at full size. These two are useful diagnostics, not working candidates.

`tools/prepare_cathedral_water_clearer_shoals.py` loads the layered scene and raises the irregular clear-patch multiplier from .42 to .72. Job `2857486` ($0.24) reveals the bright green submerged patches, but carries too much of that light into the near water. `tools/prepare_cathedral_water_graded_clarity.py` instead varies the multiplier from .30 at world y=-600 to .72 by y=-250 using a smoothstep shoreward mask. Job `2857488` ($0.16) keeps the shallows visible near the stairs and the closest water dark. This is the **current working water candidate**, still a visual study rather than a finished physical-water material. It remains subtler than the reference: the water occupies a narrow band and lacks some of the reference's irregular dark submerged silhouettes. Inspect the full image before changing camera, water geometry, or lighting. The next meaningful pass should target those silhouettes or pond shape rather than repeat another global transparency tweak.

The current PNG is `/Volumes/Hitch_07/Blender/Data/cloud-renders/renderstreet/2857488/cathedralwatergradedclaritycloudv1_00001.png`; the packed editable scene is `renders/cathedral-water-graded-clarity-cloud-v1.blend`. All four finished jobs were retrieved through `tools/download_renderstreet.py` with SHA256 manifests to the external `cloud-renders/renderstreet/<job-id>/` directories. The four jobs cost $0.70 total; observed credit balance is $20.31, above the established $5 pause point. No cloud job remains active. Source scripts and this note belong in Git, while every `.blend` and PNG stays local and ignored. No website changes or deployment were made in this water batch.

## Pond translucency comparisons — 2026-09-22

Lova asked for water with some of the reference's broken translucency. All five new tests start independently from `renders/cathedral-overgrown-right-canopy-contour-cloud-v1.blend`, the current comparison baseline. They keep the camera, forest, monument, existing object transforms and lights fixed; outputs are separate packed blends and RenderStreet stills at 1200px/64 samples. Source builders are `tools/prepare_cathedral_water_{transmission,shallows,clear_patches,submerged_mottle,shore_clarity}.py`. Rebuild each with Blender 5.2 background `--python` from this worktree; builders refuse to overwrite existing blends.

The original pond is one z=.09 plane over terrain about nine units below it. Its Principled water has roughness .30, IOR1.333 and transmission .22; the earth bed's color ramp is extremely dark. At this low viewing angle simply increasing water transmission to .58 (`2857468`, $0.15) barely changes the image. Keeping that water and brightening only the submerged terrain (`2857470`, $0.10) is also nearly invisible. Mixing irregular transparent patches into the water above that brighter bed (`2857472`, $0.10) makes faint horizontal variation, but not enough green depth. These three are diagnostic rejects, not new baselines.

`submerged_mottle` (`2857474`, $0.15) adds a thin procedural layer at z=-.7 under the water, with broken green emission between transparent gaps, and increases the clear-patch mix to .42. That is the first clearly visible depth effect, but pushes green too far into the near foreground. Camera-ray intersection with the water plane at image center measures row1080 at world y=-117, row1140 at y=-461 and bottom row1199 at y=-628. `shore_clarity` (`2857478`, $0.10) changes only the submerged-layer mask: smooth world-y fade from -625 to -180 favors the far shore while the nearest water stays dark. This is the **working water candidate** for comparison, not a final physical-water solution. Its green shoreward band is closer to the reference but still subtle and somewhat stylized. Do not silently roll it into the site; compare its full PNG with the reference and the previous baseline before further water or scene changes. Next naturalism pass could replace the thin emissive layer with irregular modeled shallow growth or a better lit near-surface bed while preserving that shoreward/foreground value split.

The full images and SHA256 manifests live outside Git under `/Volumes/Hitch_07/Blender/Data/cloud-renders/renderstreet/<job-id>/`; for the working candidate the PNG is `2857478/cathedralwatershoreclaritycloudv1_00001.png`. RenderStreet jobs all finished and were downloaded through the official FTPS helper; no browser download needed. Total listed job prices for this water batch are $0.60 and final observed credit balance is $21.01, well above the established $5 pause point. No cloud job remains active. Blender sources and this handoff are the only files to commit; all blends and render images stay ignored on Hitch_07. The main checkout handoff is mirrored from this file.
...
## Trunk, forest light, and stair marker comparisons — 2026-09-22

Lova liked the connected overhead canopy and requested several more passes without a check-in. Four independent full-frame RenderStreet comparisons start from `cathedral-overgrown-overhead-connected-cloud-v1.blend`, except the final bark correction, which starts from the trunk-value variant. Camera, monument, stairs, planting positions and the open central view stay fixed. `tools/prepare_cathedral_trunk_value.py` copies only the two colossal-trunk bark materials, lifting the dark ramp endpoint x2 and the bright endpoint x3. Job 2857434 completed at 1200px/64 samples, $0.15. The trees become more legible and feel larger, but their vertical ribs read too evenly; retain as a diagnostic, not the chosen material.

`tools/prepare_cathedral_bank_light.py` multiplies only left/right canopy-opening point light energy by 2.4/2.2. Job 2857436 completed, $0.10. The forest brightens only slightly at full frame, so leave this unpromoted. The bank already contains detailed `fine foliage` objects; weak image texture is not simply a missing-light problem. `tools/prepare_cathedral_lamp_readability.py` brightens the bronze hoods x1.9, lamp glass emission 4 to 6, and all eighteen local lamp pools x2.5. Job 2857438 completed, $0.10. Distant lamp definition improves little while foreground orange water reflections grow too strong; reject this lamp treatment.

`tools/prepare_cathedral_trunk_natural.py` loads the bright-trunk diagnostic and inserts a broad, low-frequency generated-coordinate shadow mask (factor roughly .48 to 1) on only those two bark shaders. Job 2857440 completed, $0.15. This keeps a restrained indication of immense trunk surfaces and breaks some of the repeated rib brightness; retain `renders/cathedral-overgrown-trunk-natural-cloud-v1.blend` as the current working visual candidate, not a final user-approved scene. Full PNG: `/Volumes/Hitch_07/Blender/Data/cloud-renders/renderstreet/2857440/cathedralovergrowntrunknaturalcloudv1_00001.png`. Previous connected-canopy job 2857410 remains the clean comparison. The natural-bark difference is modest; the reference still has finer, more varied forest texture and stronger integration between the colossal trunks and their surrounding foliage.

Before touching the mid-forest geometry, final-camera ray probes found frontmost `fine foliage` on most of the right bank and one clearly visible rounded `plinth planting 030` in the left mid-forest. Many other plinth centers project there but sit behind visible leaves, so do not replace them solely from projected center coordinates. A useful future geometry pass should select verified visible clumps and test enough of a region to read at full frame; avoid another tiny material or light adjustment. All four jobs finished, cost $0.50 in listed charges, and were retrieved through FTPS to Hitch. RenderStreet displayed $22.10 remaining after the last job, well above the agreed $5 pause threshold. No active render job, website edit, or media added to Git. Builders save separate packed scenes and refuse overwrite; required tests pass 24 with one skip, Python parses, and diff checks pass. The source and these notes should be checkpointed on both branches.

## Visible foliage comparisons — 2026-09-22

Lova asked for several autonomous passes while preserving the camera, monument, stairs, and colossal trunks. All four independent comparisons start from `renders/cathedral-overgrown-trunk-natural-cloud-v1.blend`; each builder saves a separate packed scene and refuses overwrite. Before choosing targets, `tools/probe_frame_cells.py` was corrected to skip camera-invisible shadow flags as well as render-hidden objects. The two old `forest shadow flag` meshes are lighting controls, not visible forest. Camera probes show that most of both banks already have detailed `fine foliage` in front; merely adding isolated small plantings behind them is unlikely to register.

`tools/prepare_cathedral_shore_hero.py` widens, shifts right, and raises the existing detailed right shoreline crown. RenderStreet job 2857442 finished at 1200px/64 samples, $0.13. Its enlarged bright mass is too round and isolated, and cuts into the right frame; do not promote. `tools/prepare_cathedral_left_plinth_foliage.py` fits detailed leafy geometry to three camera-visible rounded left-bank plinth cores without moving their screen bounds. Job 2857444 cost $0.10. The change barely registers at full frame; retain only as a diagnostic, do not combine automatically.

`tools/prepare_cathedral_shore_layered.py` adds one larger but darker detailed crown behind the original shoreline tree. Job 2857446 cost $0.10. It softens the silhouette slightly but blends into the dark bank and does not resolve the reference's broader forest character; leave unpromoted. `tools/prepare_cathedral_right_canopy_contour.py` instead reshapes twenty ray-verified visible right-bank crowns with uneven vertical/horizontal scale while preserving each crown's ground-contact height. Job 2857448 cost $0.15. This is the only comparison in the batch that changes the full-frame bank contour: vegetation rises farther around the right colossal trunk and feels closer to the reference, although some rounded crown clumps remain. Retain `renders/cathedral-overgrown-right-canopy-contour-cloud-v1.blend` as a tentative working comparison, not an explicit user-approved replacement. Full PNG: `/Volumes/Hitch_07/Blender/Data/cloud-renders/renderstreet/2857448/cathedralovergrownrightcanopycontourcloudv1_00001.png`. Keep the previous natural-bark job 2857440 as the clean baseline.

All four RenderStreet jobs finished and were downloaded by the existing FTPS tool into Hitch; listed charges total $0.48 and the displayed balance after the last job is $21.62, far above the agreed $5 pause threshold. No job remains active. The new `.blend` and PNG files remain outside Git; commit only the four builders, the probe fix, and handoff notes. Required tests passed 24 with one skip, source syntax and diff checks passed. The larger lesson is to judge at full frame: small plinth replacements and a dim extra crown barely moved the image, whereas a deliberately shaped bank region did. A next pass should refine or light the visible bank as a continuous area, rather than repeat isolated round-crown enlargements.
