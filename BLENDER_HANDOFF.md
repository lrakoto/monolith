...
- Earlier1200px full-quality baseline: `renders/cathedral-ascent-lighting-full.png` and `.blend`, built by `tools/render_cathedral_study.py` with the camera, ascent and lighting flags documented below. Latest working candidate: `cathedral-branch-patch-upper.png` and `.blend` (900px/32samples), retaining the earlier48 crowns plus24 upper-edge targets; see the return-from-break section below. Earlier both-patch scene remains intact. High-resolution crop: `cathedral-branch-patch-detail.png` and `.blend`; that blend has render border enabled. Reflection experiment `cathedral-pond-reflection-study.*` was not retained. These new outputs stay local/ignored.
