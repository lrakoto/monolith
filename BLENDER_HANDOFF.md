...
- Earlier1200px full-quality baseline: `renders/cathedral-ascent-lighting-full.png` and `.blend`, built by `tools/render_cathedral_study.py` with the camera, ascent and lighting flags documented below. Latest working candidate: `cathedral-branch-patch-both.png` and `.blend` (900px/32samples), retaining shoreline-natural plus48 visible branching crowns; see the targeted branch section below. High-resolution crop: `cathedral-branch-patch-detail.png` and `.blend`; that blend has render border enabled. Reflection experiment `cathedral-pond-reflection-study.*` was not retained. These new outputs stay local/ignored.
