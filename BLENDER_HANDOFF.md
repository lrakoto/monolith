...
Updated 2026-09-17. Read this before continuing the cinematic forest work. This is a local, editable Blender study for Lova's portfolio, not yet a website asset or deployment.
...
- Earlier1200px full-quality baseline: `renders/cathedral-ascent-lighting-full.png` and `.blend`, built by `tools/render_cathedral_study.py` with the camera, ascent and lighting flags documented below. Latest working candidate: `cathedral-branch-banks-lean-study.png` and `.blend` (900px/32samples), the104-crown banks scene with a lighter distant mesh; see the lighter geometry section below. Original full-template banks-study remains intact. Wider136-crown extended-lean-study is an unpromoted experiment. Earlier upper/both-patch scenes remain intact. High-resolution crop: `cathedral-branch-patch-detail.png` and `.blend`; that blend has render border enabled. Reflection experiment `cathedral-pond-reflection-study.*` was not retained. These new outputs stay local/ignored.
